...
class SensorInfo(socketserver.BaseRequestHandler):
...
class VideoInfo(socketserver.StreamRequestHandler):
    ...
    light_cascade = cv2.CascadeClassifier("cascade_xml/lights.xml")

    d_to_camera = CameraDist()
...
class Server(object):
    ...
    def video_stream(self, host, port):
        s = socketserver.TCPServer((host, port), VideoInfo)
        s.serve_forever()

    def sensor_stream(self, host, port):
        s = socketserver.TCPServer((host, port), SensorInfo)
        s.serve_forever()
...
if __name__ == '__main__':
    h, p1, p2 = "192.168.1.35", 8091, 8092
